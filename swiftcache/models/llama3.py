...
from swiftcache.layers.attention import Attention,Attention2
from swiftcache.layers.layernorm import RMSNorm
from swiftcache.layers.linear import QKVParallelLinear, MergedColumnParallelLinear, RowParallelLinear
...
class Llama3Attention(nn.Module):
    ...
    def __init__(
        ...
        context = get_context()
        tp_size = dist.get_world_size(context.tp_group)
        ...
        if context.role == "master" :
            self.attn_backend = Attention2
        else:
            self.attn_backend = Attention
...
class Llama3Model(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([Llama3DecoderLayer(config,layer_id) for layer_id in range(config.num_hidden_layers)])
